...
---------------------------------------------------------- */
  ...
  -------------------------------------------------------- */
    ...
    if (freshOrder && freshOrder.status !== "cancelled") {
      ...
      // --------------------------------------------------------
      // Auto-generate invoice when order becomes delivered
      // --------------------------------------------------------
      if (implied === "delivered") {
        try {
          const existingInvoice = await Invoice.findOne({
            where: { orderId: freshOrder.id, shopId: freshOrder.shop_id },
          });

          if (!existingInvoice) {
            const orderWithItems = await Order.findOne({
              where: { id: freshOrder.id, shop_id: freshOrder.shop_id },
              include: [
                {
                  model: (await import("../models/OrderItem.js")).default,
                  as: "items",
                },
                { model: Shop, as: "shop" },
              ],
            ...
            const customerObj = freshOrder.customer;
            ...
            const newInvoice = await Invoice.create({
              ...
              customerId: freshOrder.customer_id,
              shopId: freshOrder.shop_id,
              items: itemsSnapshot,
              ...
              paymentStatus:
                freshOrder.payment_status === "paid" ? "Paid" : "Unpaid",
              amountPaid:
            ...
            sendInvoiceEmail(newInvoice.toJSON()).catch((err) => {
              console.error(
                "Auto-invoice email failed (task flow):",
                err.message,
              );
            });

            console.log(
              `[AUTO-INVOICE][TASK] Invoice ${invoiceNumber} created for order #${freshOrder.id}`,
            );
          }
        } catch (invoiceError) {
          console.error("[AUTO-INVOICE][TASK] Failed:", invoiceError.message);
        }
      }
    }
