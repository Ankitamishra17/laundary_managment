...
export default function SuperAdminInvoices() {
  ...
  const load = async (silent = false) => {
    try {
      ...
      const invoicesRes = await getAllTenantInvoices(params);
      const data = invoicesRes?.data || [];
      setInvoices(data);
      setTotalPages(invoicesRes?.pagination?.totalPages || 1);

      // Extract unique shop options from the data
      if (data.length > 0) {
        const shopsMap = new Map();
        data.forEach((inv) => {
          if (inv.shop && !shopsMap.has(inv.shop.id)) {
            shopsMap.set(inv.shop.id, inv.shop);
          }
        });
        setShopOptions(Array.from(shopsMap.values()));
      }
    } catch (err) {
  ...
  }, [page, statusFilter, search, shopFilter]);
  ...
  // Summary stats (computed from loaded data for super admin)
  ...
  const summaryCards = [
    { title: "Total Invoices", value: invoices.length, icon: FileText, color: colors.primaryTeal, bg: `${colors.primaryTeal}15` },
    { title: "Total Revenue", value: formatINR(totalAmount), icon: IndianRupee, color: "#0B6E63", bg: "#DFF7F1" },
    ...
    { title: "Amount Due", value: formatINR(totalUnpaid), icon: IndianRupee, color: "#B3261E", bg: "#FBE9E8" },
  ];
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: colors.textMuted }}>
            View and manage invoices across all tenants.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by invoice number, customer, or shop..."
            value={search}
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <Store size={15} style={{ color: colors.textMuted }} />
          <select
            value={shopFilter}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="hidden md:block overflow-x-auto rounded-2xl border" style={{ borderColor: colors.cardBorder }}>
            <table className="w-full text-sm min-w-[900px]" style={{ backgroundColor: colors.bgLight }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${colors.cardBorder}` }}>
                  ...
                  <th className="text-left font-medium px-5 py-3 text-xs uppercase tracking-wide" style={{ color: colors.textMuted }}>Shop</th>
                ...
                {invoices.map((inv) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5" style={{ color: colors.textDark }}>
                          <Store size={13} style={{ color: colors.seafoam }} />
                          {inv.shop?.name || inv.shopName || "—"}
                        </div>
                      </td>
                      <td className="px-5 py-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="text-xs mb-1 flex items-center gap-1" style={{ color: colors.textMuted }}>
                    <Store size={11} className="inline" />
                    {inv.shop?.name || inv.shopName || "—"}
                  </div>
